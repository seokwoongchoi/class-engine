...
public:
	...
	const bool KeyDown(VirtualKey key) const { return keyMap[key] == static_cast<uint>(KeyStatus::KEY_INPUT_STATUS_DOWN); }
	const bool KeyUp(VirtualKey key) const { return keyMap[key] == static_cast<uint>(KeyStatus::KEY_INPUT_STATUS_UP); }
	const bool KeyPress(VirtualKey key) const { return keyMap[key] == static_cast<uint>(KeyStatus::KEY_INPUT_STATUS_PRESS); }
